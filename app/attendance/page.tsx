...
const STATUS_CONFIG: Record<
  AttendanceStatus,
  { label: string; activeClass: string; badgeBg: string; badgeText: string; badgeBorder: string }
> = {
  PRESENT: {
    label: "Present",
...
const selectCls =
  "rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none bg-white";

const inputCls =
  "rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none";
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Track student attendance for each group and date.
          </p>
        </div>
          ...
          <div className="space-y-6">
            ...
            {!markGroupId ? (
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col items-center justify-center py-16 text-gray-400">
                ...
                <p className="text-sm">Select a group to start marking attendance.</p>
              </div>
            ) : studentsLoading ? (
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    {(["PRESENT", "ABSENT", "LATE"] as AttendanceStatus[]).map((s) => (
                      <span key={s} className={`font-medium ${STATUS_CONFIG[s].badgeText}`}>
                        {statusCounts[s] ?? 0} {STATUS_CONFIG[s].label.toLowerCase()}
                      </span>
                    ))}
                  </div>
                  <span className="text-xs text-gray-400">{formatDisplayDate(markDate)}</span>
                </div>
              ...
              <div className="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
                ...
                {!histLoading && (
                  <span className="text-sm text-gray-400">
                    {histRecords.length} record{histRecords.length !== 1 ? "s" : ""}
                  </span>
                )}
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b border-gray-100">
                      <tr>
                        {["Student", "Group", "Date", "Status", "Actions"].map((h) => (
                          <th
                            key={h}
